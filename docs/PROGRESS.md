...
- [ ] Backend integration tests — **in progress (2026-08-02; see Recent Fixes #19).** Harness is live: Vitest 4 + supertest, `backend/vitest.config.ts` (node env, 20s timeouts), `backend/tests/setup.ts` (`authHeader()` mints a seeded-admin Bearer; `app` is imported from the extracted app factory `src/app.ts` so tests drive routes via supertest without spawning the BullMQ worker or binding a port). Four green, self-cleaning suites (each `afterAll` removes every row the suite touched; dev DB left spotless, 0 orphans verified across all four phone blocks):
  ...
  - **Run:** `cd backend && npm test` → 4 files / 32 passed / green ~1s. The harness + the first two suites were committed in `768fd30 "Backend testing implementation"`; the **count + create suites + the `db.create` birthday fix (Recent Fixes #20)** + these doc updates are **uncommitted** (no commit without explicit request).
...
- **`backend/src/db/database.ts` declaration-emit errors (TS4023, TS4082)** — surfaced 2026-08-03 after the JWT fix (#21) removed the auth.ts errors that were masking them (verified pre-existing: they appear with the unmodified HEAD `database.ts` once auth.ts typechecks; proven not caused by the birthday or JWT edits). `export const clients` (line 14) and the default `export { clients, messages }` (line 481) have inferred types that reference `mockDatabase`'s non-exported `Client` interface ([backend/src/db/mockDatabase.ts:30](backend/src/db/mockDatabase.ts#L30)); under `tsconfig`'s `"declaration": true`, TS can't name that private type in the generated `.d.ts` → `TS4023 "cannot be named"` + `TS4082 "private name 'Client'"`. Currently errors out (`tsc --noEmit` exit 2 on these 2 only). **Runtime + tests are unaffected** (the backend runs, 32 tests green). Minimal consistent fix (not yet applied — awaiting go/no-go): `export` the mockDatabase interfaces (`interface Client` → `export interface Client`, matching the existing precedent of `export interface Message`), since they're the data shapes the abstraction layer already exposes. Blockers: none for runtime; CI `tsc` gates would fail until fixed.
...
6. **Testing** — *in progress (2026-08-03; see Testing section + Recent Fixes #19–#20).* Harness live (Vitest + supertest + `src/app.ts` app-factory extraction). **Done:** `PUT /clients/:id` (E.164 + duplicate-409 + tax-field preservation), `DELETE /clients/:id` (soft-opt-out + already-opted-out), `GET /clients/count` (audience parity via bracketed `buildAudienceWhere`, prefix-scoped predicate matrix), and `POST /clients` (E.164 + both duplicate-409 paths incl. soft-deleted-not-reusable + clean tax-field defaults) — **32 tests green across `clients.import` + `clients.crud` + `clients.count` + `clients.create`** (4 files, ~1s; dev DB left spotless, 0 orphans across all four phone blocks). The create suite also surfaced a real `db.create` birthday-coercion bug — **fixed in Recent Fixes #20** (one-line coercion + the `it.todo` converted to a passing regression test). **Remaining, while the contracts are fresh:** `POST /campaigns` (audience validation + MANUAL-≥1 guard), `POST /campaigns/:id/send` (mock Twilio, Redis up: audience resolution + status flip + QUEUED creation + enqueue). Then frontend Jest component suites, then Playwright E2E.
