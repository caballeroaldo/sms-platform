...
### Testing
- [ ] Backend unit tests
- [ ] Backend integration tests — **in progress (2026-08-02; see Recent Fixes #19).** Harness is live: Vitest 4 + supertest, `backend/vitest.config.ts` (node env, 20s timeouts), `backend/tests/setup.ts` (`authHeader()` mints a seeded-admin Bearer; `app` is imported from the extracted app factory `src/app.ts` so tests drive routes via supertest without spawning the BullMQ worker or binding a port). Four green, self-cleaning suites (each `afterAll` removes every row the suite touched; dev DB left spotless, 0 orphans verified across all four phone blocks):
  ...
  - `backend/tests/clients.create.test.ts` (7 passing + 1 `it.todo`) — `POST /clients`: happy-path 201 + bare-10-digit→E.164 normalize + clean tax-field defaults via direct DB (all null/false so a later import's "new → create" isn't polluted); missing `firstName`/`phone`→400 with no row; invalid format (NPA 555)→400 no row; duplicate-active→409 one row; duplicate-opted-out→409 (soft-deleted phones aren't reusable via POST — locks that contract); optional-fields persist + `optedOut=false`; 401 unauth. **Phone block `+1312999`** (NPA 312). The `it.todo` records a **real bug surfaced by this suite** (see Known Issues): `db.create` real-mode passes a date-only `YYYY-MM-DD` `birthday` to Prisma's `DateTime` scalar → "premature end of input" → 500; `db.update` real-mode and mock `create` both coerce via `new Date()`, real `create` does not. Mock mode masked it (mock coerces), which is why it only appeared now.
  - **Run:** `cd backend && npm test` → 4 files / 31 passed / 1 todo / green ~1s. The harness + the first two suites were committed in `768fd30 "Backend testing implementation"`; the **count + create suites** + these doc updates are **uncommitted** (no commit without explicit request).
  - **Remaining backend:** `POST /campaigns` audience validation + MANUAL-≥1 guard; `POST /campaigns/:id/send` (mock Twilio, Redis up: status validation, audience resolution, QUEUED rows, BullMQ enqueue, no real SMS).
...
- **`POST /clients` 500s when `birthday` is a date-only string (real mode)** — surfaced by `tests/clients.create.test.ts` (2026-08-02; recorded there as an `it.todo`). `db.create` real-mode ([backend/src/db/database.ts:140](backend/src/db/database.ts#L140)) passes `data.birthday` raw to `prisma.client.create`; Prisma's `DateTime` scalar rejects a bare `YYYY-MM-DD` with "premature end of input. Expected ISO-8601 DateTime", the route throws, and the catch-all returns 500. The Add/Edit Client form renders birthday as `<input type="date">` ([frontend/lib/components/ClientForm.tsx:188](frontend/lib/components/ClientForm.tsx#L188)), whose value is exactly a date-only string — so this is reachable from the real UI (producing a confusing 500 instead of a 201). Mock mode masked it because the mock `create` ([database.ts:125](backend/src/db/database.ts#L125)) coerces via `new Date(data.birthday)`; `db.update` real-mode ([database.ts:181](backend/src/db/database.ts#L181)) coerces the same way — real `create` is the odd one out. **One-line fix** (not yet applied — awaiting go/no-go per the verify-before-applying rule): in `db.create` real-mode, mirror the others — `birthday: data.birthday ? new Date(data.birthday) : null`. When fixed, convert the `it.todo` to an asserting test sending `birthday: '1990-05-15'` and expecting 201. Blockers: none for the tests (suite is green with the `todo`); real-UI impact only when adding a client with a birthday set.
...
19. **Backend integration-test harness + first four green suites** — Lays the testing foundation and the first coverage. No source-route behavior changed (one structural extraction for testability); everything runs in mock Twilio (import/CRUD/create never enqueue or send).
   ...
   - **`tests/clients.create.test.ts` (7 passing + 1 `it.todo`, green)** — `POST /clients` on a dedicated phone block (`+1312999` prefix, NPA 312, disjoint from the seed + import + CRUD + count blocks). The API sends bare 10-digit phones (`bareFor(NN)` strips the `+1`) to exercise the route's `normalizeToE164`; fixtures are created directly via Prisma with raw E.164 (bypassing normalize, like the seed). Happy path: 201 + E.164 normalization (`3129990011`→`+13129990011`) + a direct-DB assertion that POST owns identity only and leaves the import-owned tax-season fields at clean-neutral defaults (`taxFiledDate`/`taxReturnType`/`taxpayerStatus` null; `inactive`/`clientLY`/`clientNew` false; `lastName` defaulted to `""`). Validation: missing `firstName`→400 "First name and phone are required" (no row created); missing `phone`→400; reserved NPA `555`→400 "Invalid phone number format" (no row at the would-be `+15550000000`). Duplicates: active client→409 (exactly one row); **soft-deleted (opted-out) client→409** — locks the contract that a soft-deleted phone is NOT reusable via POST (`db.findByPhone` returns opted-out rows, so POST can't "re-create" the number as fresh opted-in; the original stays opted-out with its original `firstName`, un-overwritten). Optional fields (`lastName`/`email`/`birthday`/`notes`) persist; `optedOut` defaults false. 401 unauth. `afterAll` hard-deletes the block; POST writes no audit rows.
   - **Real bug surfaced by the create suite (recorded as `it.todo`, NOT yet fixed):** `db.create` real-mode (`backend/src/db/database.ts:~140`) passes `data.birthday` raw to Prisma's `DateTime` scalar, which rejects a bare `YYYY-MM-DD` ("premature end of input") → the route throws → **500**. The form's `<input type="date">` ([frontend/lib/components/ClientForm.tsx:188](frontend/lib/components/ClientForm.tsx#L188)) sends exactly that date-only string, so this is reachable from the real UI. `db.update` real-mode (`database.ts:181`) and the mock `create` (`database.ts:125`) both coerce via `new Date()` — mock mode masked it for the project's whole life. **One-line fix** (held per the verify-before-applying rule): in `db.create` real-mode, `birthday: data.birthday ? new Date(data.birthday) : null`. See [Known Issues](#known-issues). The optional-fields test currently sends a full ISO datetime (`'1990-05-15T00:00:00.000Z'`) to stay green; the `it.todo` documents the date-only failure and the fix.
   - **Verified:** `cd backend && npm test` → 4 files / 31 tests passed / 1 todo / green ~1s; post-run sweep: 0 orphaned rows across all four phone blocks (`+14089990001`, `+1415999*`, `+1212999*`, `+1312999*`). Test-discovered bug (import/crud suites): the first run used NPA `555` (`+15559990001`), which `normalizeToE164` rejects — switched the fixtures to valid NPAs `408`/`415`/`212`/`312` (the import-route comment already warns about this).
   - **Committed** in `768fd30 "Backend testing implementation"`: the harness (`src/app.ts`, `vitest.config.ts`, `tests/setup.ts`, `package.json` devDeps, slimmed `src/index.ts`) plus the import + CRUD suites. The **count suite** and the **create suite** (`tests/clients.count.test.ts`, `tests/clients.create.test.ts`) and the PROGRESS.md edits recording both are uncommitted (no commit without explicit request).
...
6. **Testing** — *in progress (2026-08-02; see Testing section + Recent Fixes #19).* Harness live (Vitest + supertest + `src/app.ts` app-factory extraction). **Done:** `PUT /clients/:id` (E.164 + duplicate-409 + tax-field preservation), `DELETE /clients/:id` (soft-opt-out + already-opted-out), `GET /clients/count` (audience parity via bracketed `buildAudienceWhere`, prefix-scoped predicate matrix), and `POST /clients` (E.164 + both duplicate-409 paths incl. soft-deleted-not-reusable + clean tax-field defaults) — **31 tests / 1 todo green across `clients.import` + `clients.crud` + `clients.count` + `clients.create`** (4 files, ~1s; dev DB left spotless, 0 orphans across all four phone blocks). The create suite also surfaced a real `db.create` birthday-coercion bug (recorded as `it.todo` + Known Issues; one-line fix held per verify-before-applying). **Remaining, while the contracts are fresh:** `POST /campaigns` (audience validation + MANUAL-≥1 guard), `POST /campaigns/:id/send` (mock Twilio, Redis up: audience resolution + status flip + QUEUED creation + enqueue). Then frontend Jest component suites, then Playwright E2E.
